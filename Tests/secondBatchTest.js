...
fixture('UI test case 2 for current open job positions')
.page('https://company.trivago.com/open-positions/'); //opening the webpage

test('Should check for heading, button, values for second batch of current open job positions', async t => {

// Finding the count of total opened jobs
    var tableElements = Selector('tr');
    var count    = await tableElements.count;
